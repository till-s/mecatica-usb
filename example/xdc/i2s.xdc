...
create_clock -name i2sBCLK -period 434.03 [get_ports {i2sBCLK}]

# min delay after clock falling edge when pblrc becomes valid: min duty cycle after rising edge
# 
set_input_delay -clock i2sBCLK -min -add_delay 195.0 [get_ports {i2sPBLRC}]

# max delay after clock falling edge when pblrc becomes valid: max duty cycle + output delay
# 434*0.55 + 10
set_input_delay -clock i2sBCLK -max -add_delay 249.0 [get_ports {i2sPBLRC}]

# trace delay - external hold time
set_output_delay -clock i2sBCLK -min -add_delay -10.0 [get_ports {i2sPBDAT}]
...
set_false_path -from [get_clocks i2sBCLK] -through [get_pins -hier -regexp {.*B_I2S_SYNCHRONIZERS[.]rateMeasUsb2_reg[[][0-9]{1,2}[]]/D}]
